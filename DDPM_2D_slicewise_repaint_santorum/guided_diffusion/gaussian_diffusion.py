...
class GaussianDiffusion:
    ...
    def training_losses_segmentation(
        ...
        # NEW: RePainting the noisy GT
        res_t = gt_mask * res_t + (1 - gt_mask) * voided

        # NEW: x_t IS NO LONGER NEEDED SINCE THE MODEL INPUT IS THE NOISY MASKED IMAGE
        x_t = res_t.float()  # NEW: 1 channel
        # Before:
        # x_t = x_start.float()
        # x_t[:, -1:, ...] = res_t.float()  # replace last channel by noisy GT (RePainted)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # NEW: RePainting the target as well since the target is the noise added
            # by the diffussion model, so if we repaint the model output, we should
            # repaint the target as well
            target = gt_mask * target + (1 - gt_mask) * voided

            # NEW: RePainting the model output to just apply the loss on the inpainted region
            model_output = gt_mask * model_output + (1 - gt_mask) * voided
