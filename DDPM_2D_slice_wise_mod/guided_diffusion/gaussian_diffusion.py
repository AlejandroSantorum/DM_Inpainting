...
class GaussianDiffusion:
    ...
    def training_losses_segmentation(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            ###############################
            # print(f"[segm loss] Model mean type: {self.model_mean_type}")
            # print(f"[segm loss] Model var type: {self.model_var_type}")
            # print(f"[segm loss] target shape: {target.shape}")
            # print(f"[segm loss] model_output shape: {model_output.shape}")
            # print(f"[segm loss] mse: {terms['mse']}")

            # import time
            # from torchvision.utils import save_image
            # base_path = "/scratch/santorum/tmp/debug_replicate_durrer"
            # save_image(target, f"{base_path}/loss_segm_target_ts{int(time.time())}.png", normalize=True)
            # save_image(model_output, f"{base_path}/loss_segm_model_output_ts{int(time.time())}.png", normalize=True)
            ###############################
            if "vb" in terms:
