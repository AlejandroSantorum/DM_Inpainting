...
def get_model_and_diffusion(image_size):
    model_pt_path = "/scratch/santorum/checkpoints/replic_durrer_inpaint_slicewise_voided/savedmodel065000.pt"
...
def main(
    ...
    brats23_dataset = BRATSDataset(
        "/scratch/santorum/bratsc2023-dm-inpainting-preprocessed-3d/Training/",
        test_flag=True
    )

    logger.log(f"Loaded {len(brats23_dataset)} samples from BRATS23 dataset")

    sample_batch_2, sample_path_2, sample_slicedict_2 = brats23_dataset[2]  # 87
    sample_batch_4, sample_path_4, sample_slicedict_4 = brats23_dataset[4]  # 100
    sample_batch_5, sample_path_5, sample_slicedict_5 = brats23_dataset[5]  # 130
    ...
    for i in range(len(inpainted_train_batch)):
        ...
        _ground_truth_img = rotate(original_train_batch[i, 1, ...].view(actual_img_size, actual_img_size), angle=-90)
    ...
    plt.tight_layout()
    os.makedirs("/scratch/santorum/inference/replic_durrer_inpaint_slicewise_voided/gpu", exist_ok=True)
    plt.savefig(
        "/scratch/santorum/inference/replic_durrer_inpaint_slicewise_voided/gpu/inpaint_on_training_samples.png",
    )
